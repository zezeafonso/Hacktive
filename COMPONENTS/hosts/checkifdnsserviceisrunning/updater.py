...
from COMPONENTS.hosts.componentupdater import update_components_found_dns_service_is_running
...
def update_check_if_dns_service_is_running(context:dict, filtered_objects:list):
	"""
	...
	for filtered_obj in filtered_objects:
		# FOUND MSRPC is up
		if isinstance(filtered_obj, Filtered_FoundDNSServiceIsUp):
			logger.debug(f"filter for checking if DNS service is up for ip ({host.get_ip()}) found that it IS UP")

			dns_port = filtered_obj.get_port()
			update_components_found_dns_service_is_running(host, dns_port)
	return 