...
from filter import EnumDomainTrustsThroughRPC_Filter
from updater import update_enum_domain_trusts_through_rpc
...
class EnumDomainTrustsThroughRPC(AbstractMethod):
	...
	def create_run_events(context:dict=None) -> list:
		...
		return [Run_Event(type='run', filename=output_file, command=cmd, method=EnumDomainTrustsThroughRPC, context=context)]
